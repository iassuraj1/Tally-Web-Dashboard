...
export const GST_STATE_BY_CODE = Object.fromEntries(GST_STATES.map((state) => [state.code, state.name]));
export const GST_STATE_NAMES = GST_STATES.map((state) => state.name);

export const getGstState = (value = '') => {
...
export const getGstStateFromGstin = (gstin = '') => getGstState(String(gstin).trim().slice(0, 2));

export const isUnionTerritoryGstState = (value = '') => getGstState(value)?.taxType === 'UTGST';

export const getGstTaxType = (companyState = '', placeOfSupply = '') => {
...
export const splitGstAmount = (taxable = 0, gstRate = 0, taxType = 'SGST') => {
  const tax = (Number(taxable || 0) * Number(gstRate || 0)) / 100;
  if (taxType === 'IGST') return { cgst: 0, sgst: 0, utgst: 0, igst: tax };
  if (taxType === 'UTGST') return { cgst: tax / 2, sgst: 0, utgst: tax / 2, igst: 0 };
  return { cgst: tax / 2, sgst: tax / 2, utgst: 0, igst: 0 };
};
