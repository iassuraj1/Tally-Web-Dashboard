          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b">
                {['HSN Code', 'Description', 'Rate', 'UQC', 'Qty', 'Taxable Value', 'CGST', 'SGST', 'UTGST', 'IGST', 'Cess', 'Grand Total'].map((h) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {rows.map((r, i) => (
                <tr key={`${r.hsnCode}-${r.rate}-${i}`} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3 text-right font-mono">{fmt(r.utgst)}</td>
              ...
              {rows.length === 0 && <tr><td colSpan={12} className="px-5 py-10 text-center text-gray-400">No data</td></tr>}
            </tbody>
          </table>
        );
      }}
