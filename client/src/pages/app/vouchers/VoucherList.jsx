...
import { FiEye, FiXCircle, FiFilter, FiPlus } from 'react-icons/fi';

const fmt     = (n) => n ? `₹${Number(n).toLocaleString('en-IN')}` : '—';
const fmtDate = (d) => new Date(d).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });

const TYPES = ['Sales','Purchase','Payment','Receipt','Journal','Contra','CreditNote','DebitNote','StockJournal'];
...
const voucherPath = { Sales:'sales', Purchase:'purchase', Payment:'payment', Receipt:'receipt', Journal:'journal', Contra:'contra', CreditNote:'credit-note', DebitNote:'debit-note', StockJournal:'stock-journal' };
...
export default function VoucherList() {
  ...
  useEffect(load, [company, typeFilter, dateFrom, dateTo]);
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-gray-100">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-xl text-xs focus:outline-none" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {['Type','Voucher No','Date','Party / Narration','Amount','Status','Actions'].map(h => (
