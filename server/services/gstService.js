...
const dateFilter = (companyId, query = {}, extra = {}) => {
  const filter = { company: companyId, isCancelled: false, status: 'Approved', ...extra };
  if (query.from) filter.date = { $gte: new Date(query.from) };
  if (query.to) filter.date = { ...filter.date, $lte: new Date(query.to) };
  return filter;
};

const lineTax = (line) => round2(Number(line.cgst || 0) + Number(line.sgst || 0) + Number(line.igst || 0) + Number(line.cess || 0));

const calculateLine = (line, isIGST = false) => {
...
const calculateVoucherTotals = (voucherLike) => {
  const items = Array.isArray(voucherLike.items) ? voucherLike.items : [];
  const calculatedItems = items.map((item) => ({ ...item, ...calculateLine(item, voucherLike.isIGST) }));
  ...
  const total = round2(subtotal + totalCGST + totalSGST + totalIGST + totalCess + Number(voucherLike.roundOff || 0));

  return { items: calculatedItems, subtotal, totalCGST, totalSGST, totalIGST, totalCess, total };
};
...
const getVoucherTotals = (voucher) => {
  if (Array.isArray(voucher.items) && voucher.items.length) {
    ...
    return {
      ...
      total: round2(itemTaxable + itemCGST + itemSGST + itemIGST + itemCess + Number(voucher.roundOff || 0)),
    };
  }
...
const emptyTotals = () => ({ taxable: 0, cgst: 0, sgst: 0, igst: 0, cess: 0, total: 0 });
...
const classifyGstr1Section = (voucher) => {
  const partyGstin = gstin(voucher.partyGstin || voucher.party?.gstin);
  if (['CreditNote', 'DebitNote'].includes(voucher.voucherType)) return partyGstin ? 'cdnr' : 'cdnur';
  if (partyGstin) return 'b2b';
  if (voucher.isIGST && Number(voucher.total || 0) > 250000) return 'b2cl';
  return 'b2cs';
};

const getGSTR1Report = async (companyId, query = {}) => {
  const vouchers = await Voucher.find(dateFilter(companyId, query, { voucherType: { $in: OUTWARD_TYPES } }))
  ...
  const summary = { totalTaxable: 0, totalCGST: 0, totalSGST: 0, totalIGST: 0, totalCess: 0, totalTax: 0, invoiceCount: 0 };

  for (const voucher of vouchers) {
    const section = classifyGstr1Section(voucher);
  ...
  summary.totalTax = round2(summary.totalCGST + summary.totalSGST + summary.totalIGST + summary.totalCess);
...
const getGSTR3BReport = async (companyId, query = {}) => {
  ...
  for (const voucher of vouchers) {
    ...
    if (['Sales', 'CreditNote'].includes(voucher.voucherType) || debitNoteIsOutward) {
      ...
      if (!voucher.partyGstin && !voucher.party?.gstin && voucher.isIGST) addTotals(interStateUnregistered, totals, sign);
    }
  ...
  taxPayable.total = round2(taxPayable.cgst + taxPayable.sgst + taxPayable.igst + taxPayable.cess);
...
const getHSNSummary = async (companyId, query = {}) => {
  ...
  for (const voucher of vouchers) {
    ...
    for (const item of voucher.items || []) {
      ...
      hsnMap[key].total = round2(hsnMap[key].taxable + hsnMap[key].cgst + hsnMap[key].sgst + hsnMap[key].igst + hsnMap[key].cess);
    }
  }
...
const getGstMismatchReport = async (companyId, query = {}) => {
  ...
  for (const voucher of vouchers) {
    const calculated = calculateVoucherTotals(voucher);
    ...
    for (const [index, item] of (voucher.items || []).entries()) {
      ...
      const line = calculateLine(item, voucher.isIGST);
      ...
      if (voucher.isIGST && (money(item.cgst) !== 0 || money(item.sgst) !== 0)) {
        addIssue(voucher, `Line ${lineNo}: CGST/SGST present on IGST invoice`, 'CGST 0, SGST 0', `${item.cgst || 0}, ${item.sgst || 0}`);
      }
      if (!voucher.isIGST && money(item.igst) !== 0) {
...
const getMissingGstinReport = async (companyId, query = {}) => {
  ...
  for (const voucher of vouchers) {
    ...
    rows.push({
      ...
      taxable: totals.taxable,
      tax: round2(totals.cgst + totals.sgst + totals.igst + totals.cess),
      total: totals.total,
...
const validateInvoiceGst = async (payload, companyId) => {
  ...
  const calculated = calculateVoucherTotals(payload);
  items.forEach((item, index) => {
    const lineNo = index + 1;
    const line = calculateLine(item, payload.isIGST);
    if (Number(item.gstRate || 0) > 0 && !item.hsnCode) {
      errors.push(`Stock line ${lineNo}: HSN/SAC is required for taxable GST items.`);
    }
    if (payload.isIGST && (money(item.cgst) !== 0 || money(item.sgst) !== 0)) {
      errors.push(`Stock line ${lineNo}: IGST invoice cannot have CGST/SGST amounts.`);
    }
    if (!payload.isIGST && money(item.igst) !== 0) {
...
const buildEInvoiceJson = async (companyId, voucherId) => {
  ...
  return {
    ...
    TranDtls: {
      ...
      IgstOnIntra: voucher.isIGST ? 'Y' : 'N',
    },
    DocDtls: {
    ...
    ItemList: (voucher.items || []).map((item, index) => ({
      ...
      SgstAmt: formatAmount(item.sgst),
    ...
    ValDtls: {
      ...
      SgstVal: formatAmount(totals.sgst),
